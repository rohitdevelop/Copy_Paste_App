...
const Navbar = () => {
  ...
  // Correct position: Use totalTasks after it is defined
  const [prevTaskCount, setPrevTaskCount] = useState(totalTasks);
  const [newTasks, setNewTasks] = useState(0);

  // Update notification when new tasks are added
  useEffect(() => {
    if (totalTasks > prevTaskCount) {
      setNewTasks((e) => e + (totalTasks - prevTaskCount));
    }
    setPrevTaskCount(totalTasks);
  }, [totalTasks]);
